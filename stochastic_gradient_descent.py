...
def main(): 
    ...
    while(not np.array_equal(o, T_train)):
        ...
        W += (ALPHA*error_vec.transpose())*X_train
    ...
    print('\n----- STARTING TEST -----\n')
    sleep(2)
    o_test = np.sign(np.diagonal(X_test @ W.transpose()))
    error = T_test - o_test
